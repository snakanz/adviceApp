-- =====================================================
-- COMPLETE DATABASE WIPE - AGGRESSIVE
-- =====================================================
-- This script drops ALL tables and recreates clean schema
-- =====================================================

BEGIN;

-- Drop ALL existing tables (CASCADE to handle dependencies)
DROP TABLE IF EXISTS _backup_calendartoken CASCADE;
...
DROP TABLE IF EXISTS ai_document_analysis_queue CASCADE;
...
CREATE POLICY "Users can view own calendar connections" ON calendar_connections FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own meetings" ON meetings FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own clients" ON clients FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own client business types" ON client_business_types FOR ALL USING (client_id IN (SELECT id FROM clients WHERE user_id = auth.uid()));
CREATE POLICY "Users can view own pipeline activities" ON pipeline_activities FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own client todos" ON client_todos FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own client documents" ON client_documents FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own ask threads" ON ask_threads FOR ALL USING (user_id = auth.uid());
CREATE POLICY "Users can view own ask messages" ON ask_messages FOR ALL USING (thread_id IN (SELECT id FROM ask_threads WHERE user_id = auth.uid()));
CREATE POLICY "Users can view own transcript action items" ON transcript_action_items FOR ALL USING (user_id = auth.uid());
