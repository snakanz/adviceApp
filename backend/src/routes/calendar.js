...
router.post('/generate-summary-stream', authenticateSupabaseUser, async (req, res) => {
  ...
  try {
    ...
    const streamModel = streamGenAI.getGenerativeModel({ model: 'gemini-2.5-flash', generationConfig: { maxOutputTokens: 3000 } });
    ...
    if (process.env.OPENAI_API_KEY) {
      setImmediate(async () => {
        try {
          ...
          const emailModel = emailGenAI.getGenerativeModel({ model: 'gemini-2.5-flash', generationConfig: { maxOutputTokens: 3000 } });
        ...
        if (process.env.GEMINI_API_KEY) {
          setImmediate(async () => {
            try {
              ...
              const bgEmailModel = bgGenAI.getGenerativeModel({ model: 'gemini-2.5-flash', generationConfig: { maxOutputTokens: 3000 } });
              ...
              const bgPrompt = prepared.messages.map(m => m.role === 'system' ? `Instructions: ${m.content}` : m.content).join('\n\n');
              const bgResult = await bgEmailModel.generateContent(bgPrompt);
              const emailBody = bgResult.response.text() || '';
              let emailSummary;
              if (prepared.sectionConfig.includeGreetingSignOff) {
                emailSummary = emailBody.trim();
              } else {
