...
function parseAadtsError(errorCode, errorDescription) {
  ...
  if (errorCode === 'access_denied') {
    ...
    logger.log('⚠️ access_denied with no recognized consent phrase, treating as admin consent required');
...
router.get('/google/status', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    logger.error('Error in /auth/google/status:', error);
    return res.status(500).json({
      connected: false,
...
router.post('/verify-webhooks', authenticateSupabaseUser, async (req, res) => {
  try {
    const userId = req.user.id;
    logger.log(`🔍 Verifying webhooks for user ${userId} on login...`);
    ...
    try {
      ...
      if (googleConnection) {
        logger.log(`✅ Google Calendar is active for user ${userId}`);
      ...
      logger.log(`ℹ️ Google Calendar not connected for user ${userId}`);
    }
    ...
    try {
      ...
      if (microsoftConnection && microsoftConnection.microsoft_subscription_id) {
        logger.log(`✅ Microsoft Calendar is active for user ${userId}`);
        ...
        if (hoursUntilExpiry < 24) {
          logger.log(`🔄 Microsoft webhook expiring soon (${hoursUntilExpiry.toFixed(1)}h), renewing...`);
          ...
          try {
            ...
            logger.log('✅ Microsoft webhook renewed on login');
            ...
            logger.error('❌ Failed to renew Microsoft webhook:', renewError.message);
            webhookResults.microsoft = {
              status: 'error',
              sync_method: 'webhook',
      ...
      logger.log(`ℹ️ Microsoft Calendar not connected for user ${userId}`);
    }
    ...
    try {
      ...
      if (calendlyConnection) {
        logger.log(`🔍 Calendly is connected for user ${userId}, verifying webhook...`);
        ...
        if (webhookStatus.webhook_active) {
          logger.log(`✅ Calendly webhook verified active for user ${userId}`);
        } else {
          logger.warn(`⚠️ Calendly webhook not active for user ${userId} - will use polling`);
        }
      }
    } catch (err) {
      logger.log(`ℹ️ Calendly not connected for user ${userId}`);
    }
    ...
    logger.error('Error verifying webhooks:', error);
    res.status(500).json({
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    logger.log('📅 /api/auth/google/callback called');
    logger.log('  - code:', code ? '✅ Present' : '❌ Missing');
    logger.log('  - state:', state ? '✅ Present' : '❌ Missing');
    ...
    if (state) {
      ...
      if (stateData) {
        ...
        logger.log('📅 Authenticated user ID from state:', authenticatedUserId);
        logger.log('📅 Is onboarding:', isOnboarding);

        // Delete the nonce after use (one-time use)
        oauthStateStore.delete(state);
      } else {
        // State not found - could be expired or invalid (potential CSRF attack)
        logger.warn('⚠️ Invalid or expired state parameter - possible CSRF attempt');
        ...
        try {
          ...
          logger.log('📅 Using legacy state format (JSON)');
        } catch (e) {
          logger.warn('⚠️ Could not parse state parameter:', e);
        }
      }
    }
    ...
    logger.log('📅 Google OAuth callback - Google account:', userInfo.data.email);
    logger.log('📅 Google tokens received - Access token:', tokens.access_token ? 'yes' : 'no', 'Refresh token:', tokens.refresh_token ? 'yes' : 'no');
    ...
    try {
      logger.log('🔍 Verifying Google Calendar permissions...');
      ...
      logger.log('✅ Google Calendar permissions verified - found', calendarList.data.items.length, 'calendar(s)');
    } catch (calendarError) {
      logger.error('❌ Google Calendar permission verification failed:', calendarError.message);
    ...
    if (authenticatedUserId) {
      ...
      logger.log('📅 Linking calendar to existing authenticated user:', authenticatedUserId);
      ...
      if (findError || !existingUser) {
        ...
        logger.log('⚠️ User not found in users table, attempting to create from auth.users:', authenticatedUserId);
        ...
        if (!authUser) {
          logger.error('❌ Could not find user in auth.users:', authenticatedUserId);
          throw new Error('Authenticated user not found');
        }

        logger.log('✅ Found user in auth.users:', authUser.email);
        ...
        logger.log('✅ Created user record for:', user.email);
      } else {
        user = existingUser;
        tenantId = user.tenant_id;
        logger.log('✅ Found authenticated user:', user.email);
      }
    } else {
      // New user signup via Google OAuth - create/find user by email
      logger.log('📅 New user signup or login via Google OAuth');
      const { data: existingUser, error: findError } = await getSupabase()
        .from('users')
        .select('*')
    ...
    logger.log(`✅ Created default tenant ${tenantId} for user ${user.id}`);

    // Store Google tokens in calendar_connections table
    logger.log('💾 Storing Google Calendar tokens in calendar_connections...');
    ...
    if (existingConnection) {
      logger.log('✅ Updating existing Google Calendar connection...');
      ...
      if (updateError) {
        ...
        logger.log('✅ Google Calendar connection updated successfully');
        // Trigger sync and webhook setup in background (non-blocking)
        try {
          logger.log('🔄 Triggering Google Calendar sync in background...');
          const calendarSyncService = require('../services/calendarSync');
          calendarSyncService.syncUserCalendar(user.id, { timeRange: 'extended', includeDeleted: true }).then(syncResult => {
            logger.log('✅ Google Calendar sync completed:', syncResult);
          }).catch(syncError => {
            logger.warn('⚠️  Google Calendar sync failed (non-fatal):', syncError.message);
          });
        } catch (syncError) {
          logger.warn('⚠️  Failed to start background sync:', syncError.message);
        }
        // Set up webhook for real-time updates (non-blocking)
        try {
          logger.log('📡 Setting up Google Calendar webhook in background...');
          ...
          webhookService.setupCalendarWatch(user.id).then(watchData => {
            logger.log('✅ Google Calendar webhook established:', watchData?.id);
          }).catch(webhookError => {
            logger.warn('⚠️  Webhook setup failed (non-fatal):', webhookError.message);
          });
        } catch (webhookError) {
          logger.warn('⚠️  Failed to start webhook setup:', webhookError.message);
        }
      }
    } else {
      logger.log('✅ Creating new Google Calendar connection...');

      // Deactivate all other active connections for this user (single active connection per user)
      logger.log('🔄 Deactivating other active calendar connections...');
      ...
      if (deactivateError) {
        logger.warn('Warning: Could not deactivate other connections:', deactivateError);
      } else {
        logger.log('✅ Other connections deactivated');
      }
      ...
      if (createError) {
        logger.error('Error creating calendar connection:', createError);
      } else {
        logger.log('✅ Google Calendar connection created successfully');
        // Trigger initial sync in background (non-blocking)
        try {
          logger.log('🔄 Triggering initial Google Calendar sync in background...');
          const calendarSyncService = require('../services/calendarSync');
          calendarSyncService.syncUserCalendar(user.id, { timeRange: 'extended', includeDeleted: true }).then(syncResult => {
            logger.log('✅ Initial Google Calendar sync completed:', syncResult);
          }).catch(syncError => {
            logger.warn('⚠️  Initial sync failed (non-fatal):', syncError.message);
          });
        } catch (syncError) {
          logger.warn('⚠️  Failed to start background sync:', syncError.message);
        }
    ...
    if (authenticatedUserId && !isOnboarding) {
      ...
      logger.log('✅ Google Calendar connected from Settings - redirecting to Settings');
      ...
      logger.log('✅ Google Calendar connected during onboarding - redirecting to /auth/callback');
...
router.get('/microsoft/callback/test', (req, res) => {
  logger.log('🧪 Microsoft callback test endpoint hit');
  res.json({
    success: true,
...
router.get('/microsoft', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    logger.log('🔗 Generating Microsoft OAuth URL...');
    logger.log('  - Redirect URI:', microsoftService.redirectUri);
    logger.log('  - Is onboarding:', isOnboarding);
    logger.log('  - User ID:', req.user?.id || 'none');
    ...
    logger.log('✅ Microsoft OAuth URL generated successfully');
    logger.log('  - URL length:', url.length);
    logger.log('  - State extracted:', msalState ? 'yes' : 'no');
...
router.get('/microsoft/status', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    logger.error('Error in /auth/microsoft/status:', error);
    return res.status(500).json({
      connected: false,
...
Thank you!`
    ...
    logger.error('Error generating admin consent URL:', error);
    res.status(500).json({
...
router.get('/microsoft/callback', async (req, res) => {
  try {
    ...
    logger.log(`📅 MICROSOFT CALLBACK HIT - ${timestamp}`);
    logger.log('='.repeat(80));
    logger.log('  - code:', code ? '✅ Present' : '❌ Missing');
    logger.log('  - Full query params:', JSON.stringify(req.query));
    logger.log('  - error:', oauthError || 'none');
    logger.log('  - error_description:', errorDescription || 'none');
    logger.log('='.repeat(80));

    // ✅ NEW: Check for Microsoft OAuth errors (AADSTS codes) FIRST
    // This catches admin consent required, app blocked, etc.
    if (oauthError) {
      logger.log('🔒 Microsoft OAuth error detected:', oauthError);
      logger.log('   Description:', errorDescription);

      // Parse for specific AADSTS error codes
      const aadtsError = parseAadtsError(oauthError, errorDescription);

      if (aadtsError) {
        logger.log('🔒 Identified AADSTS error type:', aadtsError.type);
        logger.log('   showAdminUrl:', aadtsError.showAdminUrl);

        // Generate admin consent URL if needed
        const adminConsentUrl = aadtsError.showAdminUrl ? generateAdminConsentUrl() : null;
        logger.log('   Generated admin consent URL:', adminConsentUrl ? 'YES' : 'NO');
        ...
        if (authenticatedUserId && aadtsError.type === 'admin_consent_required') {
          try {
            logger.log('💾 Saving pending admin consent status for user:', authenticatedUserId);
            ...
            if (existingConn) {
              ...
              logger.log('✅ Updated existing connection with pending admin consent status');
            } else {
              ...
              if (userData?.tenant_id) {
                ...
                logger.log('✅ Created new connection with pending admin consent status');
              }
            }
          } catch (dbError) {
            logger.error('⚠️ Failed to save pending admin consent status:', dbError.message);
    ...
    logger.log('📅 Microsoft OAuth callback - Microsoft account:', microsoftEmail);
    logger.log('📅 Microsoft tokens received - Access token:', accessToken ? 'yes' : 'no', 'Refresh token:', refreshToken ? 'yes' : 'no');

    // ✅ CRITICAL: Verify the user actually granted calendar permissions
    // If they didn't click "Allow" on the calendar scope, the tokens won't have calendar access
    try {
      logger.log('🔍 Verifying Microsoft Calendar permissions...');
      ...
      if (!calendarResponse.ok) {
        ...
        logger.error('❌ Calendar access check failed:', calendarResponse.status, errorData);
        throw new Error('No calendar access');
      }
      ...
      logger.log('✅ Microsoft Calendar permissions verified - found', calendarData.value.length, 'calendar(s)');
    } catch (calendarError) {
      logger.error('❌ Microsoft Calendar permission verification failed:', calendarError.message);
    ...
    if (authenticatedUserId) {
      // Calendar connection for existing user - use the authenticated user from state
      logger.log('📅 Linking Microsoft calendar to existing authenticated user:', authenticatedUserId);
      const { data: existingUser, error: findError } = await getSupabase()
        .from('users')
        .select('*')
        .eq('id', authenticatedUserId)
      ...
      // Log what we found
      logger.log('👤 Microsoft callback user lookup:', {
        authenticatedUserId,
        found: !!existingUser,
      ...
      logger.log('📅 No authenticated user in state, looking up by Microsoft email:', microsoftEmail);
      const { data: existingUser, error: findError } = await getSupabase()
        .from('users')
        .select('*')
      ...
      if (existingUser) {
        user = existingUser;
        tenantId = user.tenant_id;
        logger.log('✅ Found existing user by Microsoft email:', user.email);
      } else {
        ...
        logger.error('❌ No existing user found for Microsoft email:', microsoftEmail);
    ...
    logger.log(`✅ User ${user.id} has tenant ${tenantId}`);

    // Store Microsoft tokens in calendar_connections table
    logger.log('💾 Storing Microsoft Calendar tokens in calendar_connections...');
    ...
    if (existingConnection) {
      logger.log('✅ Updating existing Microsoft Calendar connection...');
      ...
      if (updateError) {
        ...
        logger.log('✅ Microsoft Calendar connection updated successfully');
      }
    } else {
      logger.log('✅ Creating new Microsoft Calendar connection...');
      ...
      if (createError) {
        logger.error('❌ Error creating calendar connection:', createError);
        logger.error('Error details:', { code: createError.code, message: createError.message, details: createError.details, hint: createError.hint });
        ...
        logger.log('✅ Microsoft Calendar connection created successfully:', newConnection?.id);
      }
    }
    ...
    try {
      logger.log('📡 Setting up Microsoft Calendar webhook...');
      const MicrosoftCalendarService = require('../services/microsoftCalendar');
      const microsoftService = new MicrosoftCalendarService();
      await microsoftService.setupCalendarWatch(user.id);
      logger.log('✅ Microsoft Calendar webhook set up successfully');
    } catch (webhookError) {
      logger.error('❌ Failed to set up Microsoft Calendar webhook:', webhookError.message);
    ...
    try {
      logger.log('🔄 Triggering initial Microsoft Calendar sync in background...');
      const calendarSyncService = require('../services/calendarSync');
      // Don't await - let it run in background
      calendarSyncService.syncUserCalendar(user.id, { timeRange: 'extended', includeDeleted: true }).then(syncResult => {
        logger.log('✅ Initial Microsoft Calendar sync completed:', syncResult);
      }).catch(syncError => {
        logger.warn('⚠️  Initial sync failed (non-fatal):', syncError.message);
      });
    } catch (syncError) {
    ...
    if (isOnboarding) {
      ...
      logger.log('✅ Microsoft Calendar connected during onboarding - redirecting to /auth/callback');
      res.redirect(`${process.env.FRONTEND_URL}/auth/callback?success=true&provider=microsoft&onboarding=true`);
    } else {
      // Post-login calendar connection from Settings → redirect back to Settings
      logger.log('✅ Microsoft Calendar connected from Settings - redirecting to Settings');
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        logger.log('⚠️ User not found in users table, creating from req.user:', userId);
        ...
        try {
          ...
          logger.log('✅ Created user record for:', newUser.email);
    ...
    logger.error('Error in GET /onboarding/status:', error);
...
router.put('/onboarding/step', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    logger.log('=== ONBOARDING STEP UPDATE ===');
    ...
    // Log what we found
    logger.log('👤 User check for step update:', {
      userId,
      email: req.user.email,
      found: !!existingUser,
    ...
    if (checkError) {
      if (checkError.code === 'PGRST116') {
        ...
        logger.log('⚠️ User not found in users table for step update, creating:', userId);
        ...
        try {
          ...
          logger.log('✅ Created user record before step update');
        } catch (createError) {
          logger.error('❌ Error creating user for step update:', createError);
          return res.status(500).json({
        ...
        logger.error('❌ Database error checking user for step update:', checkError);
        return res.status(500).json({
    ...
    logger.error('Error in PUT /onboarding/step:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/onboarding/business-profile', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    if (userError) {
      logger.error('Error updating user business info:', userError);
      logger.error('Error details:', { code: userError.code, message: userError.message });
      return res.status(500).json({ error: 'Failed to save business profile', details: userError.message });
    }

    logger.log(`✅ Business profile saved for user ${userId}`);
    ...
    logger.error('Error in POST /onboarding/business-profile:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/onboarding/complete', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    if (!subscription) {
      logger.log(`⚠️ No subscription found for user ${userId}, creating free tier as fallback...`);
      ...
      try {
        ...
        if (createError) {
          logger.error('❌ Error creating fallback subscription:', createError);
        ...
        subscription = newSub;
        logger.log(`✅ Created fallback free subscription for user ${userId}`);
      } catch (fallbackError) {
        logger.error('❌ Fallback subscription creation failed:', fallbackError);
        return res.status(403).json({
          error: 'Subscription required',
    ...
    if (!validStatuses.includes(subscription.status)) {
      logger.warn(`⚠️  User ${userId} has invalid subscription status: ${subscription.status}`);
      return res.status(403).json({
    ...
    if (subscription.status === 'trialing' && subscription.trial_ends_at) {
      ...
      if (trialEndDate < new Date()) {
        logger.warn(`⚠️  User ${userId} trial has expired`);
        return res.status(403).json({
          error: 'Trial expired',
    ...
    logger.log(`✅ Verified subscription for user ${userId}: status=${subscription.status}`);
    ...
    logger.log(`✅ User ${userId} completed onboarding with active subscription`);
    ...
    try {
      ...
      logger.error('⚠️  Meta CAPI StartTrial failed (non-fatal):', metaError.message);
    }

    // Trigger calendar sync and webhook setup now that onboarding is complete
    try {
      logger.log('🔄 Setting up calendar webhooks and sync after onboarding completion...');
      ...
      if (calendarConnection) {
        ...
        logger.log(`📅 User has ${provider} calendar connected`);

        // Set up webhook based on provider
        if (provider === 'google') {
          try {
            logger.log('📡 Setting up Google Calendar webhook...');
            const GoogleCalendarWebhookService = require('../services/googleCalendarWebhook');
            const webhookService = new GoogleCalendarWebhookService();

            // ✅ FIX: Await webhook creation to ensure it completes successfully
            await webhookService.setupCalendarWatch(userId);
            logger.log('✅ Google Calendar webhook created successfully');
          } catch (webhookErr) {
            logger.error('❌ Failed to set up Google webhook:', webhookErr.message);
          ...
          try {
            ...
            logger.log('✅ Microsoft Calendar webhook created successfully');
          } catch (webhookErr) {
            logger.error('❌ Failed to set up Microsoft webhook:', webhookErr.message);
            // Don't fail onboarding, but log the error prominently
          }
        }
      }
      ...
      logger.log('🔄 Triggering calendar sync...');
      const calendarSyncService = require('../services/calendarSync');
      ...
      calendarSyncService.syncUserCalendar(userId, {
        ...
        logger.log('✅ Calendar sync completed after onboarding:', syncResult);
      }).catch(syncErr => {
        logger.warn('⚠️ Calendar sync failed after onboarding (non-fatal):', syncErr.message);
      });
    } catch (syncErr) {
      logger.warn('⚠️ Failed to start calendar sync after onboarding:', syncErr.message);
    ...
    logger.error('Error in POST /onboarding/complete:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/onboarding/skip-calendar', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    if (error) {
      logger.error('Error skipping calendar:', error);
    ...
    logger.error('Error in POST /onboarding/skip-calendar:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/auto-connect-calendar', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    logger.log(`📅 Auto-connecting Google Calendar for user: ${userEmail}`);
    ...
    if (!providerToken) {
      logger.log('⚠️ No provider token found in app_metadata');
      logger.log('ℹ️ This may occur when switching calendars or if user did not sign in with Google');
    ...
    logger.log('✅ Found Google provider token in Supabase Auth session');
    ...
    if (existingConnection) {
      logger.log('✅ Calendar connection already exists, updating tokens...');
    ...
    logger.log(`✅ Google Calendar auto-connected successfully: ${newConnection.id}`);
    ...
    logger.error('Error in POST /auto-connect-calendar:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/risc/events', async (req, res) => {
  try {
    logger.log('🔐 RISC Event received');
    ...
    logger.log('🔐 RISC Event details:', JSON.stringify(decodedEvent, null, 2));
    ...
    for (const [eventType, eventData] of Object.entries(events)) {
      logger.log(`🔐 Processing RISC event: ${eventType}`);
      ...
      switch (eventType) {
        case 'https://schemas.openid.net/secevent/risc/event-type/account-disabled':
          ...
          logger.log('⚠️ RISC: Account disabled for:', subjectEmail || subjectId);
        ...
        case 'https://schemas.openid.net/secevent/risc/event-type/account-enabled':
          ...
          logger.log('✅ RISC: Account re-enabled for:', subjectEmail || subjectId);
          break;
        ...
        case 'https://schemas.openid.net/secevent/risc/event-type/account-credential-change-required':
          ...
          logger.log('⚠️ RISC: Credential change required for:', subjectEmail || subjectId);
        ...
        case 'https://schemas.openid.net/secevent/risc/event-type/sessions-revoked':
          ...
          logger.log('⚠️ RISC: Sessions revoked for:', subjectEmail || subjectId);
        ...
        case 'https://schemas.openid.net/secevent/risc/event-type/tokens-revoked':
          ...
          logger.log('⚠️ RISC: Tokens revoked for:', subjectEmail || subjectId);
          await handleTokensRevoked(subjectEmail, subjectId);
          break;

        case 'https://schemas.openid.net/secevent/risc/event-type/account-purged':
          // Account was deleted
          logger.log('⚠️ RISC: Account purged for:', subjectEmail || subjectId);
          break;

        default:
          logger.log('ℹ️ RISC: Unknown event type:', eventType);
      }
    }
    ...
    logger.error('❌ Error processing RISC event:', error);
...
async function handleAccountCompromise(email, googleId, reason) {
  try {
    if (!email && !googleId) {
      logger.warn('⚠️ Cannot handle account compromise: no email or ID provided');
      return;
    }
    ...
    if (error || !user) {
      logger.log('ℹ️ RISC: User not found in our system:', email || googleId);
      return;
    }

    logger.log(`🔐 Handling account compromise for user ${user.id} (${user.email}) - reason: ${reason}`);
    ...
    if (updateError) {
      logger.error('❌ Error deactivating calendar connections:', updateError);
    } else {
      logger.log('✅ Deactivated Google calendar connections for user:', user.id);
    }
    ...
    logger.error('❌ Error in handleAccountCompromise:', err);
  }
}
...
async function handleTokensRevoked(email, googleId) {
  try {
    if (!email && !googleId) {
      logger.warn('⚠️ Cannot handle token revocation: no email or ID provided');
      return;
    }
    ...
    if (error || !user) {
      logger.log('ℹ️ RISC: User not found for token revocation:', email || googleId);
      return;
    }

    logger.log(`🔐 Handling token revocation for user ${user.id} (${user.email})`);
    ...
    if (updateError) {
      logger.error('❌ Error clearing calendar tokens:', updateError);
    } else {
      logger.log('✅ Cleared Google calendar tokens for user:', user.id);
    }

  } catch (err) {
    logger.error('❌ Error in handleTokensRevoked:', err);
  }
}
